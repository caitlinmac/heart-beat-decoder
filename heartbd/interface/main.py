"""
Takes a model based on the option set inside of .env and
predict from that model.The core function is predict(X_predict)
which runs all the modules and local_model.py
...
def load_model():
    """
    Method: loads a model on the behavior of which the global
...
def predict(X_predict = None):
    """
    Make a prediction with a provided test.
    If none is provided there will be one created by default.
    The model used depends on the MODEL_TARGET. Which is typically
    local or pickle.
